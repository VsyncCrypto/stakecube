...
- [Development guidelines](#development-guidelines)
    - [General Stakecube Core](#general-stakecube-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Stakecube Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Stakecube Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/stakecube-project/stakecube/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_stakecube
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_stakecube --log_level=test_suite
$ valgrind -v --leak-check=full src/stakecubed -printtoconsole
```
...
# A coverage report will now be accessible at `./test_stakecube.coverage/index.html`.
```
...
- BitcoinMiner : Generates SCCs (if wallet is enabled).
...
However, in open source software such as Stakecube Core, where everyone uses
...
pay attention to for reviewers of Stakecube Core code.

General Stakecube Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Stakecube Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Stakecube Core so that they can be integrated
...
file descriptors it will cause problems with STAKECUBE's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `stakecube-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `stakecube-cli` and the GUI debug console use this table to determine how to
