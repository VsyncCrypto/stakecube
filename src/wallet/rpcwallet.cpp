...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The stakecube address is the key, the numeric amount in SCC is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zsccTracker->ListMints(true, fMatureOnly, true);
        ...
        for (auto m : setMints) {
            ...
            if (m.hashStake == 0) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zsccTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zsccTracker->ListMints(true, true, true);
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Scc to convert to zSCC\n"
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzSCCTracker* zsccTracker = pwalletMain->zsccTracker.get();
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzSCCTracker* zsccTracker = pwalletMain->zsccTracker.get();
    std::set<CMintMeta> setMints = zsccTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzSCCTracker* zsccTracker = pwalletMain->zsccTracker.get();
    std::set<CMintMeta> setMints = zsccTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zsccTracker->Add(mint, true);
        count++;
...
UniValue getzsccseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "getzsccseed\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("getzsccseed", "") + HelpExampleRpc("getzsccseed", ""));
...
UniValue dzsccstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "dzsccstate\n"
    ...
    obj.push_back(Pair("dzscc_count", nCount));
...
void static SearchThread(CzSCCWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZSCC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzscc(const UniValue& params, bool fHelp)
{
    ...
    boost::thread_group* dzsccThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzsccThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzsccThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zsccTracker->GetSerialHashes());
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
...

    {
        ...
        while (nTries < 100) {
            ...
            if (fLocked) {
                if (zsccTracker->ClearSpendCache()) {
