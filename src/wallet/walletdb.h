...
public:
    ...
    bool WriteZSCCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSCCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSCCSeed_deprecated(uint256& seed);
    bool EraseZSCCSeed();
    bool EraseZSCCSeed_deprecated();

    bool WriteZSCCCount(const uint32_t& nCount);
    bool ReadZSCCCount(uint32_t& nCount);
