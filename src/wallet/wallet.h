...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zsccspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 SCC at the same time
...
enum ZerocoinSpendStatus {
    ZSCC_SPEND_OKAY = 0,                            // No error
    ZSCC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSCC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSCC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSCC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSCC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSCC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSCC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSCC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSCC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSCC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSCC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSCC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSCC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSCC_SPENT_USED_ZSCC = 14,                      // Coin has already been spend
    ZSCC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSCC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSccBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSCCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzsccAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSCCTracker> zsccTracker;
    ...
    {
        zwalletMain = zwallet;
        zsccTracker = std::unique_ptr<CzSCCTracker>(new CzSCCTracker(strWalletFile));
    }
    ...
    void setZSccAutoBackups(bool fEnabled)
    {
