...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->CreateZSCCOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zscc output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZSCCOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zscc output");
    }
