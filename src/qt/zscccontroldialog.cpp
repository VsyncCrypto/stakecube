...
#include "ui_zscccontroldialog.h"
...
std::set<std::string> ZSccControlDialog::setSelectedMints;
std::set<CMintMeta> ZSccControlDialog::setMints;

bool CZSccControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSccControlDialog::COLUMN_DENOMINATION || column == ZSccControlDialog::COLUMN_VERSION || column == ZSccControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSccControlDialog::ZSccControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSccControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSccControlDialog::~ZSccControlDialog()
{
    delete ui;
}

void ZSccControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSccControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSccControlWidgetItem* itemDenom(new CZSccControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSccControlWidgetItem *itemMint = new CZSccControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zsccTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsccTracker->GetSpendCache(mint.hashStake);
...
void ZSccControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSccControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZScc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSccControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSccControlDialog::GetSelectedMints()
{
...
void ZSccControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
