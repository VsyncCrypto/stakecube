                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zSCC ought to be enough for anybody." - Bill Gates, 2017
    ui->zSCCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSCCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSCCpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzSCC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zSCC, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSCC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSCC in ") +
...
void PrivacyDialog::on_pushButtonSpendzSCC_clicked()
{
    ...
    if(true) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zSCC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zSCC
        sendzSCC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSCC
    sendzSCC();
}

void PrivacyDialog::on_pushButtonZSccControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZSccControlDialog* zSccControl = new ZSccControlDialog(this);
    zSccControl->setModel(walletModel);
    zSccControl->exec();
}

void PrivacyDialog::setZSccControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSccSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSCC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Stakecube Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSCCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSCC is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSCC</b>";
    ...
    if (!ZSccControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSccControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSCC was spent successfully update the addressbook with the label
    ...
    // Clear zscc selector in case it was used
    ZSccControlDialog::setSelectedMints.clear();
    ui->labelzSccSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSCC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Scc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zSCC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsccTracker->GetMints(false);
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSCC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSCC "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zSCC "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSCC </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSCC </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zSCC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzSCCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzSCC->isEnabled();
    bool fButtonsEnabled = false;
    bool fMaintenanceMode = true;
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSCC
        //ui->pushButtonMintzSCC->setEnabled(false);
        //ui->pushButtonMintzSCC->setToolTip(tr("zSCC is currently disabled due to maintenance."));

        // Spend zSCC
        ui->pushButtonSpendzSCC->setEnabled(false);
        ui->pushButtonSpendzSCC->setToolTip(tr("zSCC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSCC
        //ui->pushButtonMintzSCC->setEnabled(true);
        //ui->pushButtonMintzSCC->setToolTip(tr("PrivacyDialog", "Enter an amount of SCC to convert to zSCC", 0));

        // Spend zSCC
        ui->pushButtonSpendzSCC->setEnabled(true);
        ui->pushButtonSpendzSCC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
