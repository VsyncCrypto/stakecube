...
CzSCCWallet::CzSCCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zscc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSCCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSCCCount(nCountLastUsed);
    else if (!walletdb.ReadZSCCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSCCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSCC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
void CzSCCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsccTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSCCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSCC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsccTracker which also adds to database
    pwalletMain->zsccTracker->Add(dMint, true);
...
void CzSCCWallet::SeedToZSCC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
void CzSCCWallet::GenerateDeterministicZSCC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
