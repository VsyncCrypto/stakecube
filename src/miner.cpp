        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zsccs that have been in mempool long
                    //and higher priority to zsccs that are large in value
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("STAKECUBEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zsccTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zsccTracker->ListMints(true, true, true); //update the state
        }
        return error("STAKECUBEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("STAKECUBEMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running STAKECUBEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("STAKECUBEMiner exception");
    } catch (...) {
        LogPrintf("STAKECUBEMiner exception");
    }

    LogPrintf("STAKECUBEMiner exiting\n");
}
